...
这句口令现在会安装或更新全局 skill：`$CODEX_HOME/skills/codex-with-cc`，未设置 `CODEX_HOME` 时使用 `~/.codex/skills/codex-with-cc`。

安装后重启 Codex。后续在任意项目里提到子代理、子线程、多代理、委派、派工、执行层，或者显式使用 `$codex-with-cc`，都会强制走这套工作流。
